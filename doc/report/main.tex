...
% TODO: possibly add image of a divering simulation
...
Firstly, notice the structs defined in the \icd{lib} directory -- namely \icd{Grid}, \icd{vec2d}, \icd{Point}. The \icd{Grid} struct is a generic two-dimensional array of fixed size and is used to represent the values of a quantity at the grid points inside the simulated container. The \icd{vec2d} struct is a representation of a two-dimensional vector (with two coordinates of type \icd{double}) and \icd{Point} of a point with two integer coordinates. All these structs make heavy use of operator overloading, so that they can be used more easily and naturally. That includes a natural way of addressing the data in a \icd{Grid}, so that statements like
\begin{align*}
	\icd{g(3, 4) = g(0, 1)}
\end{align*}
or even
...
Our \software{} contained a bug which sometimes lead to a segmentation fault during the closing of the program. Since this did not interfere with the \software{}'s functionality in any way, fixing the crash was not a priority until later in the development process. It then turned out that the crash occurs inside the destructor of gtkmm's \icd{Gtk::Switch}, which internally calls some methods of the GTK's \icd{GtkSwitch} widget.

At that time, there was already a report\footnote{See \cite{GtkmmSwitchReport}.} of an issue in gtkmm that demonstrated the crash on a much simpler example. After some debugging of the gtkmm and GTK libraries, I concluded that there was a bug in the underlying GTK library, which leads to a null pointer being dereferenced whenever the destruction method is called on a \icd{GtkSwitch} that is still inside of some window. This has been reported\footnote{See \cite{GtkSwitchReport}.} in the GTK issue tracker on March 5, 2021 and has not been fixed as of today. Note that this bug only affects GTK+ 3 and not the newer but incompatible GTK 4.
...
\begin{enumerate}
	...
	\item
		Let CMake build the project. The project defines two CMake targets: \icd{brandy0} (main application) and \icd{brandy0-test} (tests). \icd{brandy0} builds the project normally, whereas building \icd{brandy0-test} produces an executable that only runs some tests and not the main application.
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=.5]{../shots/2021-04-07_start_screen.png}
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=.35]{../shots/2021-04-07_config_screen.png}
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=.3]{../shots/2021-04-07_shape_config_screen.png}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth]{../shots/2021-04-07_simulation_screen_u.png}
	\caption{The simulation window with the \emph{velocity magnitude} visual.}
\end{figure}

\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth]{../shots/2021-04-07_simulation_screen_p.png}
	\caption{The simulation window with the \emph{pressure} visual.}
\end{figure}

\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth]{../shots/2021-04-07_simulation_screen_u_2.png}
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=.4]{../shots/2021-04-07_video_export_screen.png}
